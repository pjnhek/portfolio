...
**Domain:** Developer portfolio site (Next.js + Tailwind + TypeScript on Vercel Hobby)
**Researched:** 2026-05-20
**Confidence:** HIGH

## Executive Summary

For a recruiter-facing AI engineer portfolio in May 2026, the inevitable stack is:
...
Three things to actively avoid: Tailwind v3 (already legacy), `next lint` (removed in v16 — use ESLint flat config directly), and any UI kit heavier than shadcn/ui's vendored Radix primitives (no MUI, no Chakra, no Ant — they wreck Lighthouse and signal "template").
...
pnpm create next-app@latest pjnhek --typescript --tailwind --eslint --app --src-dir --import-alias "@/*"

cd pjnhek

# Supporting libraries
pnpm add clsx tailwind-merge lucide-react zod resend sonner

# Optional — only if you commit to MDX project pages
pnpm add @next/mdx @mdx-js/loader @mdx-js/react gray-matter
# OR
pnpm add next-mdx-remote-client gray-matter

# Optional — only if you actually need motion beyond CSS
pnpm add motion

# Dev tooling
pnpm add -D prettier prettier-plugin-tailwindcss eslint-config-prettier @types/node @types/react @types/react-dom
```

**Sanity-check versions after install:**

```bash
pnpm list next react tailwindcss typescript
# Expect:
#   next            16.2.x
#   react           19.2.x
#   tailwindcss     4.1.x
...
**If you stay strictly minimal (PROJECT.md current scope):**
...
**If you add a live RAG demo (deferred v2):**
...
**If recruiters click on mobile (per PROJECT.md, they do):**
...
Type   Name   Value
A      @      76.76.21.21         # apex (RFC1034 prohibits CNAME at apex)
...
Set the apex (`pjnhek.com`) as the canonical domain and redirect `www` → apex in Vercel's domain settings. This avoids SEO duplicate-content penalties and makes the link cleaner in resumes.
...
**Goal:** Lighthouse 100 / 100 / 100 / 100 on desktop, 95+ across the board on mobile. With this stack and no UI kit, this is the default outcome, not a stretch.
...
---
*Stack research for: AI engineer personal portfolio on Next.js + Tailwind + TypeScript + Vercel Hobby*
*Researched: 2026-05-20*
